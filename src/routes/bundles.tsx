...
export const Route = createFileRoute("/bundles")({
  head: () => ({
    meta: [
      { title: "Bundles — Save up to 67% | Edventure Printables" },
      {
        name: "description",
        content:
          "Save up to 67% with complete themed collections: Treasure Island pirates, Unicorn Isles and Galaxy Maths — every adventure from Year 3 to Year 6.",
      },
      { property: "og:title", content: "Bundles — Save up to 67%" },
      {
        property: "og:description",
        content: "Complete themed maths collections for parents, teachers and homeschoolers.",
      },
    ],
  }),
    ...
    <div className="min-h-dvh bg-warm-white text-navy">
      ...
      <header className="relative overflow-hidden border-b border-navy/5 bg-gradient-to-br from-ocean to-navy px-4 sm:px-6 py-16 sm:py-24 text-white">
        ...
        <div className="mx-auto max-w-4xl text-center relative">
          ...
          <h1 className="mt-5 font-display text-4xl sm:text-6xl font-extrabold leading-tight">
            Save up to <span className="text-gold">67%</span> with our bundles
          </h1>
          <p className="mt-4 mx-auto max-w-2xl text-lg text-white/80">
            Complete themed collections — a whole world of adventures from Year 3 to Year 6 in one
            download, or every world at once.
          </p>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-16 grid gap-6 sm:gap-8 md:grid-cols-2">
        {bundles.map((b) => (
          <div
            key={b.slug}
            className="group overflow-hidden rounded-3xl bg-white shadow-xl shadow-navy/5 ring-1 ring-navy/5 card-lift"
          >
            ...
            <div className="p-6 sm:p-8">
              ...
              <h2 className="mt-2 font-display text-2xl sm:text-3xl font-extrabold text-navy">
                {b.title}
              </h2>
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                {b.includes.map((s) => (
                  <span
                    key={s}
                    className="rounded-full bg-sand-soft px-2.5 py-1 text-xs font-bold text-navy"
                  >
                    {s}
                  </span>
                ))}
      ...
      <section className="px-4 sm:px-6 py-16 sm:py-20 bg-sand-soft/50">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mt-3 text-lg text-navy/70">
            Mix &amp; match any packs — the more you add, the more you save.
          </p>
          ...
          <Link
            to="/shop"
            className="mt-8 inline-block rounded-2xl bg-navy px-7 py-4 font-display text-lg font-extrabold text-white hover:bg-ocean-deep"
          >
            Start shopping
          </Link>
        </div>
